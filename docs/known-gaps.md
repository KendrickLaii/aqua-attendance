...
> 最後更新：2026-08-28（已審查：2026-07-28；2026-08-27 補學費發票／課程計價缺口；2026-08-28 堂費按上課日計堂；文首補 **#F1** ERP 路線 → [erp-roadmap.md](erp-roadmap.md)）
...
> 計價在 SKU、班次名冊、按月 Generate、堂費按上課日∩出勤計堂**已做**。下面兩項是明確延後的。細節見 #M22–#M24。

| # | 尚未做 | 現況 | 影響 |
|---|--------|------|------|
| **#M22** | 下學年重報同一班號 | `(unit_id, sku_id)` **永久唯一**；取消後再報仍 409 | 不能保留舊報名又開新學年 A1 |
| **#M23** | 堂費扣公眾假期 | Generate 已按上課日∩該據點出勤計堂；不減假期 | 假期仍可能被當成應上課日（未到則已扣） |
| **#M24** | 把帳單發給家長 | 只有後台 `draft` → `issued` → `paid` | 沒有 WhatsApp／電郵／列印發送 |
| — | Vuexy `/apps/invoice` | AQUA 模板假資料 | **不是**學費系統（見 **D5**） |

建議下一件產品工作：**#M22 唯一約束** → **#M24 發送**。假期日曆可另開。

---

## 以後／ERP（不是現在的 bug）

> **2026-08-28** — 學費月結已上線。系統以後可能長成 ERP（庫存、採購、資產）。家具等物料**不要**進 `course_skus` 或學費 Generate。路線與階段見 **[erp-roadmap.md](erp-roadmap.md)**（SSOT）。追蹤號 **#F1**。

| # | 項目 | 狀態 |
|---|------|------|
| **#F1** | ERP／庫存／家具主檔 | ⬜ 規劃。未排期。見 [erp-roadmap.md](erp-roadmap.md) |
...
- **位置**：`apps/api/app/services/tuition_invoice_generator.py`
- **已做**：堂費 `quantity` = 上課日 ∩ 該 SKU 地點（無地點則任何地點）的香港日曆日出勤。空上課日仍為 1、不看出勤。當月完全沒打卡 → 跳過該行。作廢事件不算。月費忽略上課日與出勤。
- **尚未**：沒有公眾假期表。`schedule_note` 仍是自由文字，不算堂。
- **影響**：假期若學生沒來，會當缺席扣掉；若假期仍打卡則仍會出堂。
- **建議**：需要時加假期日曆，從上課日集合先剔除。
