...
> **狀態**：規劃。不是現在要實作的功能。  
> **寫於**：2026-08-28  
> **為什麼有這份**：學費月結已經上線。之後系統可能長成 ERP（庫存、採購、資產、不只補習班收款）。家具等物料**不要**塞進 `course_skus`／報名／學費 Generate。
...
| 何時開工 | **有真實採購或銷售流程再 A** | 現在做主檔沒人用會爛 |

待產品再定（開工 Phase D 前）：賣家具是併進該月學費票，還是獨立銷售發票。
...
- [ ] Phase A migration `items`；Web 一個極簡物料頁
- [ ] Phase B 入出庫；據點當倉庫是否夠用
- [ ] 若賣給學生：先改 Generate **不刪** 非課程行，再允許加行
- [ ] 不要從 Vuexy `/apps/invoice` 抄業務

**不要做**：把 `billing_unit` 加一個 `furniture`；用報名表代表「買了桌子」。
