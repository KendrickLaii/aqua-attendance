...
@pytest.mark.asyncio
async def test_product_requires_home_and_allowed_locations(
...
@pytest.mark.asyncio
async def test_product_home_and_allowed_locations(
    client: AsyncClient,
    admin_token: str,
    ...
    create = await client.post(
        ...
        json={
            ...
            "home_location_id": sample_location["id"],
            "allowed_location_ids": [sample_location["id"], sample_location_b["id"]],
        },
        headers=headers,
    )
    assert create.status_code == 201
    body = create.json()
    assert body["home_location_id"] == sample_location["id"]
    assert body["home_location"]["name_en"] == sample_location["name_en"]
    assert set(body["allowed_location_ids"]) == {sample_location["id"], sample_location_b["id"]}
    assert len(body["allowed_locations"]) == 2

    patch = await client.patch(
        f"/api/products/{body['id']}",
        json={"allowed_location_ids": [sample_location_b["id"]], "home_location_id": sample_location_b["id"]},
        headers=headers,
    )
    assert patch.status_code == 200
    updated = patch.json()
    assert updated["home_location_id"] == sample_location_b["id"]
    assert updated["allowed_location_ids"] == [sample_location_b["id"]]
