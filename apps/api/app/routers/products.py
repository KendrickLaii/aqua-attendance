...
async def create_product(body: ProductCreate, _admin: AdminOnly, db: DB) -> ProductOut:
    ...
    _home, allowed = await product_svc.resolve_product_locations(

        db,

        home_location_id=body.home_location_id,

        allowed_location_ids=body.allowed_location_ids,

    )



    product_data = body.model_dump(exclude={"allowed_location_ids"})
...
async def update_product(product_id: uuid.UUID, body: ProductUpdate, _admin: AdminOnly, db: DB) -> ProductOut:
    ...
    allowed_ids = update_data.pop("allowed_location_ids", None)

    home_location_id = update_data.pop("home_location_id", None)
    ...
    if home_location_id is not None or allowed_ids is not None:

        resolved_home_id = home_location_id if home_location_id is not None else product.home_location_id

        resolved_allowed_ids = (

            allowed_ids

            if allowed_ids is not None

            else [loc.id for loc in product.allowed_locations]
        ...
        _home, allowed = await product_svc.resolve_product_locations(
            ...
            home_location_id=resolved_home_id,

            allowed_location_ids=resolved_allowed_ids,

        )

        product.home_location_id = resolved_home_id

        await product_svc.replace_allowed_locations(product, allowed)
